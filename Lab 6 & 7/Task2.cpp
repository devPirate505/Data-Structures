...
class BinarySearchTree{
    ...
    BinarySearchTree concatinate(node* root, BinarySearchTree t);
    public:
    BinarySearchTree(){
        root= NULL;
    }
    ...
    BinarySearchTree concatinate(BinarySearchTree tree){
    	concatinate(this->root, tree);
	}
};
int main (){
    ...
    cout << "\nSeond Tree after concatination: \n";
    tree2.InOrderTraversal();
...
BinarySearchTree BinarySearchTree:: concatinate(node* r, BinarySearchTree tree){
	if(r == NULL){
		return tree;
	}
	tree.Insert(r->data);
	concatinate(r->left, tree);
	concatinate(r->right, tree);
}
